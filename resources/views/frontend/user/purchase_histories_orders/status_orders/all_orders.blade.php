                            ...
                            <td class="text-right">
                                @if (!\App\MyClasses\OrdersNotes::check_order_note($order->id) && $delivery_status != 'refund')
                                    <button onclick="open_modal_mote({{ $order->id }})"
                                        ...
                                        data-order_id="{{ $order->id }}" title="{{ translate('Put Notes') }}">
                                        <i class="las la-sticky-note"></i>
                                    </button>
                                @endif


                                @if ($delivery_status != 'refund' && $delivery_status != 'delivered' && $delivery_status != 'cancelled')
                                    <button onclick="update_dliever({{ $order->id }})"
                                        class="btn btn-soft-danger btn-icon btn-circle btn-sm update_delivery_status"
                                        data-order_id="{{ $order->id }}" title="{{ translate('Cancel Order') }}">
                                ...
                                @if ($delivery_status == 'delivered')
                                    @if (empty($order->refund_request))
                                        @php
                                            $now = Carbon::now();
                                            $differDays = $now->diffInDays($order->created_at);
                                        @endphp
                                        @if ($differDays <= get_setting('refund_days'))
                                            <button onclick="open_modal_refund({{ $order->id }})"
                                                class="btn btn-soft-danger btn-icon btn-circle btn-sm "
                                                data-title="{{ translate('Refund Order') }}">
