            ...
            <ul class="aiz-side-nav-list" data-toggle="aiz-side-menu">
                ...
                @if (!auth()->check())
                    <li class="aiz-side-nav-item">
                        <a href="{{ route('user.login') }}"
                            class="aiz-side-nav-link {{ areActiveRoutes(['user.login']) }}">
                            <i class="las la-sign-in-alt aiz-side-nav-icon"></i>
                            <span class="aiz-side-nav-text">{{ translate('Login Or Register Now') }}</span>
                        </a>
                    </li>
                @endif
                ...
                <li class="aiz-side-nav-item">
                    <a href="{{ route('cart') }}" class="aiz-side-nav-link {{ areActiveRoutes(['cart']) }}">
                        ...
                        <span class="aiz-side-nav-text">{{ translate('Cart') }}</span>
                    </a>
                </li>
                ...
                <li class="aiz-side-nav-item">
                    <a href="{{ route('wishlists.index') }}"
                        ...
                        <span class="aiz-side-nav-text">{{ translate('My wishlists') }}</span>
                    </a>
                </li>

                <li class="aiz-side-nav-item">
                    <a href="{{ route('ourbranches') }}"
                        class="aiz-side-nav-link {{ areActiveRoutes(['ourbranches']) }}">
                        <i class="las la-map-marker-alt aiz-side-nav-icon"></i>
                        <span class="aiz-side-nav-text">{{ translate('our branches') }}</span>
                    </a>
                </li>
                    ...
                    @if (Auth::user()->user_type == 'seller')
                        ...
                        <li class="aiz-side-nav-item">
                            <a href="{{ route('orders.index') }}"
                                ...
                                @if ($orders > 0)
                                    <span class="badge badge-inline badge-success">{{ $orders }}</span>
                                @endif
                            </a>
                        </li>
                        ...
                        <li class="aiz-side-nav-item">
                            <a href="{{ route('reviews.seller') }}"
                                ...
                                @if ($review_count > 0)
                                    <span class="badge badge-inline badge-success">{{ $review_count }}</span>
                                @endif
                            </a>
                        </li>
        ...
        {{-- @if (\App\BusinessSetting::where('type', 'vendor_system_activation')->first()->value == 1 && Auth::user()->user_type == 'customer')
            <div>
                <a href="{{ route('shops.create') }}" class="btn btn-block btn-soft-primary rounded-0">
                    <span class="seller_font">{{ translate('Be A Seller') }} <i
                            class="las la-store-alt"></i></span>
                </a>
            </div>
        @endif
        @if (Auth::user()->user_type == 'seller')
                ...
                <div class="text-center">
                    <div class="heading-4 strong-700 mb-4">
                        @php
                            ...
                            // ->select(DB::raw("SUM(order_details.grand_total) as grand_total"))
                            // ->get();
                            ...
                            $orderTotal = \DB::table('orders')
                                ...
                                ->where('orders.created_at', '>=', $days_ago_30)
                                ->select(DB::raw('SUM(order_details.price +order_details.shipping_cost +  order_details.tax ) as grand_total'))
                                ->groupBy('orders.id')
                                ->get();
                        @endphp
                        <small
                            class="d-block fs-12 mb-2">{{ translate('Your sold amount (current month)') }}</small>
                        <span class="btn btn-primary fw-600 fs-18">
                            {{ single_price(count($orderTotal) != 0 ? $orderTotal[0]->grand_total : 0) }}
                        </span>
                    </div>
                        ...
                        <tr>
                            @php
                                ...
                                $orderTotal = \DB::table('orders')
                                    ...
                                    ->where('order_details.seller_id', Auth::user()->id)
                                    ->where('orders.payment_status', 'paid')
                                    ->where('orders.created_at', '>=', $days_ago_60)
                                    ->where('orders.created_at', '<=', $days_ago_30)
                                    ->select(DB::raw('SUM(order_details.price +order_details.shipping_cost +  order_details.tax ) as grand_total'))
                                    ->groupBy('orders.id')
                                    ->get();
                            ...
                            <td class="p-1 fw-600" width="40%">
                                {{ single_price(single_price(count($orderTotal) != 0 ? $orderTotal[0]->grand_total : 0)) }}
                            </td>
                        </tr>
                    </table>
                </div>
