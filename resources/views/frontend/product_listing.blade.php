...
@endsection
@php
$max_price_products = \App\Product::get()->max('unit_price');
...
@section('content')
                            ...
                            <div class="collapse-sidebar c-scrollbar-light text-left">
                                ...
                                <div class="bg-white shadow-sm rounded mb-3">
                                    ...
                                    <div class="p-3">
                                        <div class="aiz-range-slider">
                                                ...
                                                <div class="col-6 text-right">
                                                    <span class="range-slider-value value-high fs-14 fw-600 opacity-70"
                                                        ...
                                                        @elseif($products->max('unit_price') > 0)
                                                            data-range-value-high="{{ $max_price_products }}"
                                                        @else
                                                            data-range-value-high="0" @endif
                                                        id="input-slider-range-value-high"></span>
                                                </div>
                                            </div>
                                            <div id="input-slider-range" data-dir="{{ dir_lang() }}"
                                                data-range-value-min="@if (count(\App\Product::query()->get()) < 1) 0 @else {{-- {{ filter_products(\App\Product::query())->get()->min('unit_price') }} --}} 0 @endif"
                                                data-range-value-max="@if (count(\App\Product::query()->get()) < 1) 0 @else {{ $max_price_products }} @endif">
                                            </div>
                                            @php
                                                if (isset($min_price)) {
                                                    $min_price = $min_price;
                                            ...
                                            <div class="row d-flex justify-content-between mt-2">
                                                <div class="col-6">
                                                    <input class="form-control" value="{{ $min_price }}"
                                                        name="min_price" />
                                                </div>
                                                <div class="col-6 text-right">
                                                    <input class="form-control" value="{{ $max_price }}"
                                                        name="max_price" />
                                                </div>
                                            </div>
                                ...
                                @foreach ($attributes as $key => $attribute)
                                    @if (\App\Attribute::find($attribute['id']) != null)
                                        <div id="according" class="bg-white shadow-sm rounded mb-3">

                                            {{-- <div class="btn btn-primary" data-toggle="collapse"
                                                data-target="#demo{{ $key }}">Simple collapsible</div>
                                            <div id="demo{{ $key }}" class="collapse show">
                                            ...
                                            <div style="cursor: pointer;"
                                                class="fs-15 attr_collaspe p-3 fw-600 border-bottom " data-toggle="collapse"
                                                data-target="#collapseOne{{ $attribute['id'] }}" aria-expanded="false"
                                                aria-controls="collapseOne{{ $attribute['id'] }}">
                                                {{ translate('Filter by') }}
                                                {{ \App\Attribute::find($attribute['id'])->getTranslation('name') }}
                                                <i style="margin-inline: 10px" id="arrow_toggle{{ $key }}"
                                                    class="las la-angle-down "></i>
                                            </div>
                                            @php
                                                // $select_id = isset($selected_attributes[$key]) ? $selected_attributes[$key]['id'] : 0;
                                                $flag_show = false;
                                            ...
                                            @endphp
                                            <div class=" p-2 collapse @if ($flag_show) show @endif"
                                                data-key="{{ $attribute['id'] }}"
                                                id="collapseOne{{ $attribute['id'] }}" data-parent="according">
                                ...
                                <button type="submit"
                                    class="btn btn-styled btn-block btn-base-4 btn-primary">{{ translate('Apply filter') }}</button>
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-9">
                        ...
                        <div class="text-left">
                            <div class="d-flex align-items-center">
                                ...
                                <div class="form-group ml-auto mr-0 w-200px d-xl-block">
                                    ...
                                    <select class="form-control form-control-sm aiz-selectpicker" data-live-search="true"
                                        ...
                                        @if (count(\App\MyClasses\Cat::get_brands($products)) > 0)
                                            @foreach (\App\MyClasses\Cat::get_brands($products) as $brand)
                                        ...
                                        <div class="p-md-3 p-2 text-left">
                                            <div class="fs-15">
                                                @if (home_base_price($product->id) != home_discounted_base_price($product->id))
                                                    <del class="fw-600 opacity-50 mr-1" style="{{ get_setting("style_price_del") }}">{{ home_base_price($product->id) }}</del>
                                                @endif
                                                <span style="{{ get_setting("style_price") }}" class="fw-700 text-primary">{{ home_discounted_base_price($product->id) }}</span>
                                            </div>
                                            ...
                                            <h3 class="fw-600 fs-13 text-truncate-2 lh-1-4 mb-0">
                                                <a href="{{ route('product', $product->slug) }}" class="d-block text-reset">{{  getTranslation($product->id) == null?$product->name:getTranslation($product->id)  }}</a>
                                            </h3>
    ...
    @else
        <div class="d-flex" style="justify-content: center;align-items: center;height: 50%;">
            <h3 style="color:#7e7e7e;font-weight: bold">{{ translate('Not Found Products') }}
            </h3>
        </div>
    @endif
...
<script type="text/javascript">
    ...
    // $(".attr_collaspe").click(function(e) {
    //     let key = $(this).data("key");

    //     if ($(`#collapseExample${key}`).hasClass("show")) {
    //         $(`#collapseExample${key}`).collapse({
    //             toggle: false
    //         })
    ...
    $(".attr_collaspe").click(function(e) {
        ...
        if ($(`#collapseOne${key}`).hasClass("show")) {
            $(`#arrow_toggle${key}`).removeClass("la-angle-down")
            $(`#arrow_toggle${key}`).addClass("la-angle-up")
        } else {
            $(`#arrow_toggle${key}`).removeClass("la-angle-up")

            $(`#arrow_toggle${key}`).addClass("la-angle-down")
        }
    });
