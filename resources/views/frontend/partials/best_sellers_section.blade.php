@if (\App\BusinessSetting::where('type', 'vendor_system_activation')->first()->value == 1)
    @php
        ...
        // }
        // asort($array);
    // $sellers = \DB::select("SELECT SUM(products.num_of_sale) as num_of_sale , sellers.id FROM `sellers`
    //  JOIN products ON products.user_id = sellers.user_id
    //  WHERE sellers.verification_status = 1
    //  GROUP BY sellers.id
    //  ORDER BY num_of_sale DESC
    //  LIMIT 20");
    // $array = [];
    // $ids = [];
    ...
    // }
     ...
     $sellers_data = \DB::select("SELECT SUM(products.num_of_sale) as num_of_sale ,
      sellers.id,COUNT(reviews.id) as total,
      SUM(reviews.rating) as rating
      FROM `sellers`
      JOIN products ON products.user_id = sellers.user_id
      JOIN reviews ON reviews.product_id = products.id
      WHERE sellers.verification_status = 1
      GROUP BY sellers.id
      ORDER BY num_of_sale
      DESC LIMIT 20
");
    $array = [];
    $sellers = [];

    $sellers = collect($sellers_data)->map(function($item)
    {
     $item =    \App\Seller::find($item->id);
        ...
        <div class="container">
            <div class="px-2 py-4 px-md-4 py-md-3 bg-white shadow-sm rounded">
                ...
                <div class="aiz-carousel gutters-10 half-outside-arrow" data-items="3" data-lg-items="3"  data-md-items="2" data-sm-items="2" data-xs-items="1" data-rows="2">
                    @php
                        $count = 0;
                    // $array =  \App\Seller::whereIn('id', [$ids])->get();
                                    ...
                                    <div class="col-8 border-left border-light">
                                        <div class="p-3 text-left">
                                            ...
                                            <div class="rating rating-sm mb-2">
                                                @if ($seller->total > 0)
                                                    {{ renderStarRating($seller->rating/$seller->total) }}
                                                @else
                                                    {{ renderStarRating(0) }}
                                                @endif
                                            </div>
