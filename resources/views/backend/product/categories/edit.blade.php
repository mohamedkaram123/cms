            ...
            <div class="card">
                <div class="card-body p-0">
                    ...
                    <form class="p-4" action="{{ route('categories.update', $category->id) }}" method="POST"
                        ...
                        <div class="form-group row">
                            <label class="col-md-3 col-form-label">{{ translate('Attributes') }}</label>
                            <div class="col-md-9">
                                <select class="select2 form-control aiz-selectpicker" name="attributes[]"
                                    multiple="multiple" data-toggle="select2" data-placeholder="Choose ..."
                                    data-live-search="true">
                                    @foreach (App\Attribute::all() as $attribue)

                                        <option
                                            @foreach ($category->attributes as $attribute_child) @if ($attribute_child->attribute_id == $attribue->id)
                                             selected @endif
                                            @endforeach value="{{ $attribue->id }}">
                                            {{ $attribue->getTranslation('name') }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
