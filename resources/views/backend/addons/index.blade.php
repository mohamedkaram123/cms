...
@section('content')
    <div class="card">
        <div class="card-header">
            <h1>{{ translate('Update System') }}</h1>
        </div>
        <div class="card-body">
            <form class="form-horizontal" action="{{ route('update.system') }}" method="POST"
                enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                    <label for="exampleFormControlFile1">{{ translate('Install File') }}</label>
                    <input type="file" name="install_file" accept=".php" class="form-control-file"
                        id="exampleFormControlFile1" required>
                </div>


                <div class="text-right">
                    <button type="submit" class="btn btn-primary">{{ translate('go to update') }}</button>
                </div>
            </form>
            {{-- <div class="row ">
            <div class="col-md-6">
                                        ...
                                        <div class="align-items-center d-flex flex-column flex-md-row">
                                            ...
                                            <div class="ml-auto mr-0">
                                                <label class="aiz-switch mb-0">
                                                    <input type="checkbox" onchange="updateStatus(this, {{ $addon->id }})" <?php if ($addon->activated) {
    echo 'checked';
} ?>>
                                                    <span></span>
                                                </label>
