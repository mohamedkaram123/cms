                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-lg-6">
                            <select class="form-control aiz-selectpicker" name="symbol_format">
                                <option value="1" @if(\App\BusinessSetting::where('type', 'symbol_format')->first()->value == 1) selected @endif>{{\App\Language::where('code', \Session::get('locale', \Config::get('app.locale')))->first()->rtl == 1?"[Amount] [Symbol]":"[Symbol] [Amount]"}}</option>
                                <option value="2" @if(\App\BusinessSetting::where('type', 'symbol_format')->first()->value == 2) selected @endif>{{\App\Language::where('code', \Session::get('locale', \Config::get('app.locale')))->first()->rtl == 1?"[Symbol] [Amount]":"[Amount] [Symbol]"}}</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-group row">
    						<label class="col-md-3 col-from-label">{{ translate('currency styleSheet') }}</label>
                            <div class="col-md-8">
        						<input type="hidden" name="types[]" value="style_price">
        						<textarea style="background:#333;color:#fff;direction:ltr" name="style_price" rows="5" class="form-control" placeholder="<style>&#10;example : font-size:24px;&#10;</style>">{{ get_setting('style_price') }}</textarea>
                                <small>{{ translate('Write style without <style> tag') }}</small>
                            </div>
    					</div>
                        <div class="form-group row">
    						<label class="col-md-3 col-from-label">{{ translate('currency del styleSheet') }}</label>
                            <div class="col-md-8">
        						<input type="hidden" name="types[]" value="style_price_del">
        						<textarea style="background:#333;color:#fff;direction:ltr" name="style_price_del" rows="5" class="form-control" placeholder="<style>&#10;example : font-size:24px;&#10;</style>">{{ get_setting('style_price_del') }}</textarea>
                                <small>{{ translate('Write style without <style> tag') }}</small>
                            </div>
    					</div>
                    <div class="form-group row">
