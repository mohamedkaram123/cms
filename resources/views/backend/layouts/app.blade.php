...
/> --}}
    ...
    <link rel="stylesheet" href="{{ static_asset('assets/css/custom-style-admin.css?version=' . $version) }}">
    ...
    @php
        echo get_setting('header_script_admin');
    @endphp

</head>

<body class="">
    {{-- {{ dd(Cat::children_cat(10))}} --}}

    <div data-user_id="{{ auth()->user()->id }}" id="notify_wrapper">
    ...
    <script src="{{ static_asset('js/' . $backend_file . '.js?version=' . $version) }}"></script>
    ...
    <script type="text/javascript">
        ...
        $("#select_search").change(function() {
            ...
            if ($(this).val() == "products") {
                ...
                $("#type_search").val("products");
                $("#input_search").attr("placeholder",
                    "{{ translate('Search By') .' , ' .translate('Product Name') .' , ' .translate('Categorey Name') .' , ' .translate('Product Desc') }}"
                )
            }
            if ($(this).val() == "sales") {
                $("#type_search").val("sales");

                $("#input_search").attr("placeholder",
                    "{{ translate('Search By') .' , ' .translate('Order Id') .' , ' .translate('Order Code') .' , ' .translate('Customer Name') }}"
                )
            }
            if ($(this).val() == "customers") {
                $("#type_search").val("customers");

                $("#input_search").attr("placeholder",
                    "{{ translate('Search By') . ' , ' . translate('Customer Name') . ' , ' . translate('Phone Number') }}"
                )
            }
            if ($(this).val() == "sellers") {
                $("#type_search").val("sellers");

                $("#input_search").attr("placeholder",
                    "{{ translate('Search By') . ' , ' . translate('Seller Name') . ' , ' . translate('Seller Phone') }}"
                )
            }
    ...
    @php
        echo get_setting('footer_script_admin');
    @endphp
