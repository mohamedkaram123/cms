        ...
        <div class="row gutters-5">
            ...
            <div class="col-md-3 ml-auto">
                ...
                <select class="form-control aiz-selectpicker"  data-minimum-results-for-search="Infinity" id="update_delivery_status">
                    @foreach (App\Models\OrderStatus::where("show",1)->orderBy("arrange","asc")->get() as $status )
                            <option value="{{$status->status}}" @if ($delivery_status == $status->status) selected @endif>{{translate($status->status)}}</option>
                    @endforeach
                ...
                <table>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-right">
                                @if($status == 'delivered')
                                <span id="badge_order" class="badge badge-inline badge-success">{{ translate(ucfirst(str_replace('_', ' ', $status))) }}</span>
                                @else
                                <span id="badge_order" class="badge badge-inline badge-info">{{ translate(ucfirst(str_replace('_', ' ', $status))) }}</span>
                                @endif
                            </td>
                        </tr>
                        <tr>
