...
class BusinessSettingsController extends Controller
{
    ...
    {
        foreach ($request->types as $key => $type) {
            if ($type == "theme_cat" && $request[$type] == null) {
                $request[$type] = "0";
            }
            if ($type == 'site_name') {
        ...
        $this->create_object_genenral_trans($request);
        flash(translate("Settings updated successfully"))->success();
        return back();
    }


    public function create_object_genenral_trans($request)
    {

        $trans_datas = [
            [
                "trans_column" => "home_banner3_txt_link",
                "table_name" => "business_settings_home_banner3"
            ],
            [
                "trans_column" => "home_banner3_header_imgs",
                "table_name" => "business_settings_home_banner3"
            ]

        ];

        foreach ($trans_datas as $trans_data) {
            if ($request->has($trans_data["trans_column"])) {
                if (count($request->{$trans_data["trans_column"]}) != 0) {
                    foreach (json_decode(get_setting($trans_data["trans_column"]))  as $key => $value) {
                        create_objsects_general_transes($trans_data["table_name"], $key, $trans_data["trans_column"], $value);
                    }
                }
            }
        }
    ...
    public function updateActivationSettings(Request $request)
    {
        ...
        if ($business_settings != null) {
            ...
            if ($request->type == "multy_vendors") {
                if ($request->value == 1) {
                    $products = DB::table('products')->where("user_id", "!=", 1)->update(['published' => 1]);
                } else {
                    $products = DB::table('products')->where("user_id", "!=", 1)->update(['published' => 0]);
                }
            }
    ...
    public function update_footer(Request $request)
    {
        ...
        $type_links =  ["terms_conditions_page", "return_policy_page", "support_policy_page", "privacy_policy_page"];

        foreach ($request->footer_links_title as $key => $item) {
            Pages::update_page_name($item, $type_links[$key]);
        }
        foreach ($request->types as $key => $type) {
    ...
    public function save_setting_paytabs_egypt(Request $request)
    {
        $business_settings = BusinessSetting::where('type', "PAYTAB_PROFILE_ID")->first();
        $business_settings->value = $request->PAYTAB_PROFILE_ID;
        $business_settings->save();
        $business_settings = BusinessSetting::where('type', "PAYTAB_SERVER_KEY")->first();
        $business_settings->value =  $request->PAYTAB_SERVER_KEY;
        $business_settings->save();
        $business_settings = BusinessSetting::where('type', "paytab_sandbox")->first();
        $business_settings->value = $request->paytab_sandbox;
        $business_settings->save();
        $business_settings = BusinessSetting::where('type', "pay_tab_defualt")->first();
        $business_settings->value = $request->pay_tab_defualt;
        $business_settings->save();
        // flash(translate("Settings updated successfully"))->success();
    ...
    public function save_setting_paytabs_saudi(Request $request)
    {
        $business_settings = BusinessSetting::where('type', "PAYTAB_PROFILE_ID_saudi")->first();
        $business_settings->value = $request->PAYTAB_PROFILE_ID;
        $business_settings->save();
        $business_settings = BusinessSetting::where('type', "PAYTAB_SERVER_KEY_saudi")->first();
        $business_settings->value =  $request->PAYTAB_SERVER_KEY;
        $business_settings->save();
        $business_settings = BusinessSetting::where('type', "paytab_sandbox_saudi")->first();
        $business_settings->value = $request->paytab_sandbox;
        $business_settings->save();
        $business_settings = BusinessSetting::where('type', "pay_tab_defualt")->first();
        $business_settings->value = $request->pay_tab_defualt;
    ...
    public function save_setting_fawry(Request $request)
    {
        $business_settings = BusinessSetting::where('type', "fawry_merchant_code")->first();
        $business_settings->value = $request->fawry_merchant_code;
        $business_settings->save();
        $business_settings = BusinessSetting::where('type', "fawry_security_key")->first();
        $business_settings->value =  $request->fawry_security_key;
        $business_settings->save();
        $business_settings = BusinessSetting::where('type', "fawry_sandbox")->first();
        $business_settings->value = $request->fawry_sandbox;
        $business_settings->save();

        $business_settings = BusinessSetting::where('type', "fawry_live")->first();
        $business_settings->value = $request->fawry_live;
        $business_settings->save();
        return response()->json([
            "data" => "done"
        ]);
    ...
    public function sms_setting_index(Request $request)
    {
        return view("backend.sms_setting.sms_setting");
    }

    public function send_sms(Request $request)
    {
        if ($request->data["sms_provider"] == "provider_fekra") {
            $user = User::find($request->user_id);
            $send_data =  SMS_Send($user->phone, $request->data["sms_content"]);
